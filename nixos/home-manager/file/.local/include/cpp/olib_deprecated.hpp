...
////////////////////////////////////////////////////////////
// olib: library for easier development, c++20stdlib wrapper
////////////////////////////////////////////////////////////
...
fstream get_file(string filename) {
  fstream file;
...
auto open(string filename, bool writing=false) {
  fstream file;
  file.open(filename, writing ? std::ios::out : std::ios::in);
  if (!file) die("Failed to open file ", filename);
  return file;
}
...
auto contains(auto& container, auto& elem) { return find(container, elem); }
...
auto range(int from, int to) { // inclusive discrete ranges [from, ..., to]
  auto v = vector<int>(max(from,to) - min(from,to) + 1);
  std::iota(v.begin(), v.end(), min(from, to));
  if (from > to) std::reverse(v.begin(), v.end());
  return v;
}

auto range_(int from, int to) { // python type ranges [ from, ..., to )
  if (from < to) return range(from, to - 1);
  if (from > to) return range(from, to + 1);
  return vector<int>();
}

/* auto ran_(auto l, auto r) { return std::ranges::iota_view(int(l), int(r)); } */
/* auto ran(auto l, auto r) { return std::ranges::iota_view(int(l), int(r)); } */

/* auto ran(auto n) { return ran(0, n - 1); } */

// deprecated:
auto range_(int to) { return range_(0, to); } // python type range [ 0, ..., to )

// use instead:
auto range(int to) { return range_(0, to); } // python type range [ 0, ..., to )
